...
import { acquireSlotHold, releaseSlotHold, getSlotHoldStatus } from "../services/slotLockService.js";

const holdSlot = async (req, res) => {
    try {
        const { userId, docId, slotDate, slotTime, ttlSeconds } = req.body;
        if (!userId || !docId || !slotDate || !slotTime) {
            return res.status(400).json({
                success: false,
                message: "Missing required slot hold parameters",
            });
        }
        ...
        if (
            ...
            return res.status(400).json({
                success: false,
                message: "Slot already permanently booked by another user.",
            });
        }

        const lockTtl = Number(ttlSeconds) > 0 ? Math.min(Number(ttlSeconds), 600) : 300;
        const lockResult = await acquireSlotHold(docId, slotDate, slotTime, userId, lockTtl);

        if (!lockResult.acquired) {
            return res.status(409).json({
                success: false,
                message: lockResult.message || "Slot currently held by another patient. Please choose a different slot.",
                ttl: lockResult.ttl || 0,
            });
        }
        ...
        return res.status(200).json({
            ...
            ttlSeconds: lockResult.ttl,
        });
    } catch (error) {
        console.error("Hold slot error:", error);
        return res.status(500).json({
            success: false,
            message: "Failed to reserve slot. Please try again later.",
        });
    }
};
...
const releaseSlot = async (req, res) => {
    try {
        ...
        if (!userId || !docId || !slotDate || !slotTime) {
            return res.status(400).json({
                success: false,
                message: "Missing required release parameters",
            });
        }

        await releaseSlotHold(docId, slotDate, slotTime, userId);
        return res.status(200).json({
            success: true,
...
const bookAppointment = async (req, res) => {
    ...
    let lockAcquiredForBooking = false;
    ...
    try {
        ...
        const lockResult = await acquireSlotHold(docId, slotDate, slotTime, userId, 60);
        if (!lockResult.acquired) {
            return res.status(409).json({
                success: false,
        ...
        try {
            ...
            if (userData.email) {
                sendBookingConfirmation(userData.email, {
                    ...
                    consultationType: type,
                    videoCallLink,
                }).catch((err) => console.error("Booking queue email failed:", err));
            }
        ...
        if (lockAcquiredForBooking) {
            await releaseSlotHold(docId, slotDate, slotTime, userId).catch(() => {});
        }
...
const cancelAppointment = async (req, res) => {
    try {
        ...
        if (userEmail) {
            const docName =
                typeof appointmentData.docData === "string"
            ...
            }).catch((err) => console.error("Cancellation email queue failed:", err));
            ...
            if (refundStatus === "initiated") {
                sendRefundNotification(userEmail, refundAmount, {
                    ...
                    slotDate: appointmentData.slotDate,
                    slotTime: appointmentData.slotTime,
                }).catch((err) => console.error("Refund email queue failed:", err));
            }
        }
...
export { holdSlot, releaseSlot, bookAppointment, listAppointment, cancelAppointment };